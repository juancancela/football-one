...
class NavigationBar extends React.Component {
    ...
    renderSectionButton(option, selectedOption) {
        ...
        return (
            <button key={`${option.label}-nav-btn`}
                type="button"
                className={`btn ${isSelectedSection ? 'btn-success' : 'btn-primary'} app-btn`}>
                <Link className={s.link} to={option.link}>{option.label}</Link>
            </button>
        )
    };
    ...
    renderSectionButtons(selectedOption) {
        let menuOptions = [];
        MENU_OPTIONS.forEach(option => menuOptions.push(this.renderSectionButton(option, selectedOption)));
        return (
            <div>
                {menuOptions}
            </div>
